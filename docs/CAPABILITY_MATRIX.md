...
Objective production-readiness and domain done criteria live in `docs/REFERENCE_RUBRIC.md`.
...
| Tensor/cube primitives | batched cube kernels + higher-rank `ArrayD` ops (last-axis reductions, axis permutation, explicit-axis contraction, N-D batched last-two matmul) + rank-3 HOSVD + N-D HOSVD + N-D HOOI + Tucker projection/expansion utilities + rank-3 CP-ALS + N-D CP-ALS + CP diagnostics/reports + TT-SVD + TT orthogonalization/rounding + TT algebra (`inner`/`norm`/`add`/Hadamard) + einsum-style binary contractions | `nabled-linalg::tensor` | Implemented | Yes | Required v1 tensor surface is explicit and complete across allocating/view/into forms for required operation families (real/complex where applicable); post-v1 tensor depth now includes CP-ALS rank-3/N-D with diagnostics/reporting APIs, HOSVD/HOOI N-D with Tucker projection/expansion utilities, TT-SVD decomposition, TT orthogonalization/rounding, and TT algebra APIs. |
| Accelerator contracts | compile-time backend trait + per-operation kernel trait dispatch + CPU execution/chunking + feature-gated GPU kernels (`wgpu`) + explicit CPU fallback behavior when GPU execution is unavailable | `nabled-linalg::accelerator` | Implemented | Yes | Required v1 kernel families are wired through compile-time backend dispatch over `CpuBackend`/`GpuBackend`, including sparse (`CSR matvec`, sparse-dense matmat, sparse-sparse matmat), triangular solve kernels, expanded native `f32`, conditional native `f64` (`SHADER_F64`), and complex tensor kernels via real-kernel decomposition; see `docs/V1_STABILITY.md` for the support matrix. |
...
| Tree FK | branched URDF-origin FK/Jacobian | `nabled-kinematics::tree` | Implemented | No | `link_transforms_tree`, `jacobian_tree`, S22 Y-branch fixture. URDF models go straight to tree FK; serial `ChainSpec` only when `DhParams` is explicit (D-MOD-2). |
| Robot model | joints/links/URDF → `ChainSpec` | `nabled-model` | Implemented | No | quick-xml URDF v2, tree BFS, inertials/limits/axis, JSON fixture loader. `BodySpec::dh_params: Option<DhParams>`; URDF leaves it `None`; `to_chain_spec` fails loudly when DH is absent (D-MOD-2). |
| Dynamics | RNEA/CRBA/forward dynamics | `nabled-dynamics` | Implemented | No | Serial RNEA + CRBA mass matrix + FD; `DynamicsConfig`; S5/S5b pass. Tree-branch variants `rnea_tree`/`mass_matrix_tree`/`forward_dynamics_tree` (+ `_into`) ship S26/S27 parity (D-MOD-3). |
| Control | DARE/LQR/gramians/pole placement | `nabled-control` | Implemented | No | pole/observer/gramians/discrete gramians + `dare_residual`/`dare_residual_norm_view`; S9/S10/S15/S16/S21 pass. |
