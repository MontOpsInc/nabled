...
| View / copy contract | Rust is view-first across hot paths | View-based dense Python APIs now admit strided / Fortran-order NumPy inputs, and direct NumPy vector/matrix/tensor primitive kernels can now also reuse caller-provided output buffers; higher-level result/tensor/sparse paths still materialize owned arrays in places | Partial | Preserve zero-copy ingress where possible, keep output-buffer reuse explicit on hot kernels, and document every remaining unavoidable materialization point |
| Allocation control | Rust exposes `*_into` and reusable workspace/result helpers in hot domains | Direct NumPy vector/matrix/tensor primitive kernels now expose `out=` for caller-provided output reuse, but there is still no broader Python workspace / reusable-plan equivalent across the remaining hot domains | Partial | Expand Python allocation-control semantics beyond the current direct dense kernel slices and preserve explicit reuse/copy rules on every performance-critical path |
...
2. Allocation-control semantics are still incomplete outside the landed direct NumPy vector/matrix/tensor primitive slices, especially where Rust already exposes reusable workspaces or reusable result objects.
