...
| `sylvester` / `lyapunov` | Real and complex solves, mixed-precision variants, `*_into`, reusable workspace semantics | `sylvester_solve` and `lyapunov_solve` now accept `complex128` as well as real `float32` / `float64`, and both rows now expose caller-provided `out=` buffers under the existing Python names where the Rust core already admits `*_into` execution | Partial | Add mixed-precision coverage and Python reusable-workspace/reuse-object equivalents |
| `matrix_functions` | Real and complex `exp`, `log`, `power`, `sign` families with `*_into` and workspace-backed variants | Direct complex parity now covers admitted `complex128` rows for `matrix_exp`, `matrix_exp_eigen`, `matrix_log_eigen`, `matrix_log_svd`, `matrix_power`, and `matrix_sign`; real rows still accept both `float32` and `float64`, preserve real output dtype, and now expose caller-provided `out=` buffers for `matrix_exp`, `matrix_log_taylor`, `matrix_log_eigen`, `matrix_log_svd`, `matrix_power`, and `matrix_sign` where the Rust core already has `*_into` coverage | Partial | Add the remaining admitted complex/configurable rows and keep `matrix_exp_eigen` allocation-only until the Rust core admits an `into`/workspace equivalent or narrows admission |
...
| View / copy contract | Rust is view-first across hot paths | View-based dense Python APIs now admit strided / Fortran-order NumPy inputs, direct NumPy vector/matrix/tensor primitive kernels can reuse caller-provided output buffers, and the current higher-level SVD helper / matrix-function / Sylvester-Lyapunov rows now also expose `out=` where the Rust core already has `*_into` coverage; higher-level result/tensor/sparse paths still materialize owned arrays in places | Partial | Preserve zero-copy ingress where possible, keep output-buffer reuse explicit on hot kernels and admitted higher-level helpers, and document every remaining unavoidable materialization point |
| Allocation control | Rust exposes `*_into` and reusable workspace/result helpers in hot domains | Direct NumPy vector/matrix/tensor primitive kernels now expose `out=` for caller-provided output reuse, and the currently admitted higher-level SVD helper / matrix-function / Sylvester-Lyapunov rows now do the same where the Rust core already admits `*_into`; there is still no broader Python reusable-workspace / reusable-plan equivalent across the remaining hot domains | Partial | Expand Python allocation-control semantics beyond the current direct dense kernels plus landed higher-level helper rows, and preserve explicit reuse/copy rules on every performance-critical path |
...
2. Allocation-control semantics are still incomplete outside the landed direct NumPy vector/matrix/tensor primitive slices plus the currently landed higher-level SVD helper / matrix-function / Sylvester-Lyapunov rows, especially where Rust already exposes reusable workspaces or reusable result objects.
