...
| Result object fidelity | Rust returns typed result structs for decompositions and higher-level workflows | Python now returns typed result objects across current decomposition, dense config/result families (including QR config helpers, LU log-determinant, and non-symmetric bi-eigen results), batched, PCA/regression, tensor, Arrow SVD, and reusable sparse factorization/preconditioner workflows; sparse iterative reuse and the direct sparse convenience rows now also preserve ILUT/ILUK config metadata on the factorization objects themselves, and the remaining fidelity gaps are concentrated in config/result-rich dense/callback iterative workflows plus provider-bound mixed solve metadata not yet exposed from Python | Partial | Preserve release-relevant metadata and finish the remaining config/result-rich solver workflows |
...
2. Missing Arrow/PyArrow breadth: nearly the full admitted `nabled::arrow` surface.
3. Missing allocation-control semantics for hot Python workflows.
4. Missing truthful provider/backend feature exposure.
5. Remaining result-fidelity gaps are now narrower: config/result-rich iterative workflows still need production-grade result contracts.

## Implementation Order Derived From This Matrix

1. `N-PY-003`: continue from the landed ML + tensor parity passes:
   - remaining result-fidelity gaps in higher-level workflows, especially config/result-rich iterative and callback-driven rows
   - any remaining dense parity rows that are not blocked on `N-PY-005` feature/build exposure
