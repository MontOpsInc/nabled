...
| `svd` | Real and complex decomposition, toleranced/truncated variants, pseudo-inverse, null space, reconstruction, rank, condition number, view-first execution | Real decomposition/truncated/pinv/reconstruct/rank/condition/null-space now accept both `float32` and `float64`, return typed `SvdResult` objects, and preserve real output dtype on array-valued paths | Partial | Add complex parity and toleranced/configurable entrypoints |
| `qr` | Real and complex full/reduced/pivoted QR, least-squares, reconstruction, condition number | Real decomposition + least-squares now accept both `float32` and `float64` and return typed `QrResult` objects | Partial | Add reduced/pivoted/reconstruct/condition surfaces and complex parity |
| `lu` | Real and complex decompose/solve/inverse/determinant/log-determinant plus mixed-precision solve helpers | Real decompose/solve/inverse/determinant now accept both `float32` and `float64`, with decomposition returning typed `LuResult` objects | Partial | Add complex parity, log-determinant, and mixed-precision policy |
| `cholesky` | Real and complex decompose/solve/inverse with view and `*_into` coverage | Real decompose/solve/inverse now accept both `float32` and `float64`, with decomposition returning typed `CholeskyResult` objects | Partial | Add complex parity and allocation-control semantics for repeated solves |
| `eigen` | Real symmetric/generalized/non-symmetric, balancing, left/right bi-eigen, complex non-symmetric | Real symmetric/generalized/non-symmetric now accept both `float32` and `float64`, return typed result objects, and non-symmetric Python egress preserves complex arrays directly (`complex64` for `float32` inputs) instead of split real/imag buffers | Partial | Add balancing, bi-eigen, and complex non-symmetric parity beyond the current real-only entrypoint set |
| `schur` | Real and complex Schur decomposition | Real Schur now accepts both `float32` and `float64` and returns typed `SchurResult` objects | Partial | Add complex parity |
| `polar` | Real and complex polar decomposition | Real polar now accepts both `float32` and `float64` and returns typed `PolarResult` objects | Partial | Add complex parity |
| `sylvester` / `lyapunov` | Real and complex solves, mixed-precision variants, `*_into`, reusable workspace semantics | Real `sylvester_solve`, `lyapunov_solve` now accept both `float32` and `float64` | Partial | Add complex parity, mixed-precision coverage, and Python allocation-control/workspace equivalent |
| `matrix_functions` | Real and complex `exp`, `log`, `power`, `sign` families with `*_into` and workspace-backed variants | Real `matrix_exp`, `matrix_exp_eigen`, `matrix_log_taylor`, `matrix_log_eigen`, `matrix_log_svd`, `matrix_power`, `matrix_sign` now accept both `float32` and `float64` and preserve real output dtype | Partial | Add complex parity and equivalent allocation-control semantics |
...
|---|---|---|---|---|
| Real dtype breadth | Broad `f32` + `f64` across linalg/ml/tensor domains | Meaningful `f32` breadth is now landed across the current dense/vector/decomposition/function/batched/sparse/ML baseline; remaining gaps are concentrated in tensor breadth, callable `jacobian` / `optimization`, and Arrow-admitted rows | Partial | Expose admitted `f32` release surface wherever Rust already admits it |
...
1. Missing cross-domain numerics: `f32` parity is now meaningful across the current real-valued dense/sparse/decomposition/function/ML baseline, but release gaps remain in tensor breadth, callable `jacobian`/`optimization`, and Arrow-admitted flows.
...
1. `N-PY-003`: continue from the landed ML + tensor parity passes:
   - remaining `f32` breadth (tensor, callable, Arrow-admitted rows)
   - sparse breadth
