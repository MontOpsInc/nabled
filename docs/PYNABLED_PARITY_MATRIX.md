...
| `tensor` | Cube kernels, higher-rank last-axis ops, axis permutation/contraction, N-D batched matmul, real/complex breadth, HOSVD/HOOI/Tucker/CP/TT/einsum families | Real cube kernels, last-axis ops, permutation/contraction, N-D batched matmul, rank-3 `hosvd3` + reconstruction | Partial | Add admitted tensor decomposition/network breadth, `f32` + complex parity where Rust admits it, and define Python result-object contracts |
| `iterative` | Real and complex dense `conjugate_gradient` / `gmres` with config + view-first paths | Real `conjugate_gradient`, `gmres` only | Partial | Add complex parity and explicit config exposure |
| `jacobian` | Numerical Jacobian, central Jacobian, gradient, Hessian | No Python surface | Missing | Add full admitted Jacobian family |
| `optimization` | Line search, gradient descent, Adam, momentum, RMSProp, projected GD, SGD, BFGS, real + complex | No Python surface | Missing | Add full admitted optimization family with config objects/contracts |
| `pca` | Real and complex `compute`, `transform`, `inverse_transform` | Real `compute_pca` only | Partial | Add transform/inverse-transform and complex parity |
| `regression` | Real and complex linear regression with canonical result structs | Real `linear_regression` only | Partial | Add complex parity and preserve canonical result metadata |
| `stats` | Real and complex means/centering/covariance/correlation | Real means/centering/covariance/correlation only | Partial | Add complex parity |
...
| Complex breadth | Broad complex parity across dense, tensor, and ML/stat domains; limited to admitted Rust APIs | Almost entirely absent; current binding is effectively real-only | Partial | Expose admitted complex release surface or explicitly narrow Rust admission first |
| View / copy contract | Rust is view-first across hot paths | Python rejects non-C-contiguous arrays up front and often materializes owned results | Partial | Preserve zero-copy ingress where possible and make unavoidable copies explicit and documented |
...
1. Missing domains: `jacobian`, `optimization`.
2. Missing cross-domain numerics: broad `f32` and complex parity.
3. Missing sparse breadth: formats, products, solver/preconditioner depth, reuse paths.
4. Missing tensor breadth: post-rank-3 decomposition/network surfaces admitted by Rust.
5. Missing higher-level ML parity: PCA transform/inverse-transform, complex regression/stats.
6. Missing Arrow/PyArrow breadth: nearly the full admitted `nabled::arrow` surface.
7. Missing allocation-control semantics for hot Python workflows.
8. Missing truthful provider/backend feature exposure.

## Implementation Order Derived From This Matrix

1. `N-PY-003`: close missing domains and numerics first:
   - `jacobian`
   - `optimization`
   - complex parity in currently exposed domains
   - `f32` breadth
   - highest-value sparse/tensor gaps
