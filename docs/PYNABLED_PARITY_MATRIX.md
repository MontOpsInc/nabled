...
| `arrow` / `PyArrow` | Full admitted `nabled::arrow` surface across dense, sparse, tensor, ML, and Arrow-admitted result contracts | Python Arrow now exposes a first substantial real slice beyond the earlier baseline: vector dot/norm/cosine plus pairwise and batched row kernels, dense matvec/matmat and batched row/broadcast matmat, fixed-shape-tensor batched matrix outputs, matrix statistics (`column_means`, `center_columns`, covariance, correlation), and `arrow_svd_decompose`; these rows accept both `float32` and `float64`, Arrow-native outputs stay Arrow-native where natural, and SVD still returns typed `SvdResult` NumPy egress where the Rust Arrow facade itself is ndarray-native | Partial | Reach the remaining admitted `nabled::arrow` parity using canonical `ndarrow` carriers across decomposition, sparse, tensor, ML, and complex-arrow families |
...
| Arrow carriers | Rust uses canonical `ndarrow` carriers across admitted Arrow workflows | Python Arrow bridge now handles primitive arrays, fixed-size-list dense matrices, and fixed-shape-tensor batch carriers for the currently exposed real dense/stat rows, with Python wrappers hiding `(field, storage)` mechanics for fixed-shape tensors; sparse, variable-shape tensor, and complex-extension carrier families are still mostly unbound | Partial | Use the same canonical `ndarrow` carrier set and explicit ingress/egress contracts as Rust |
| PyArrow egress | Rust Arrow facade can stay Arrow-native where natural | Python now keeps Arrow-native egress for the currently exposed real dense/vector/matrix/stats rows, including fixed-shape-tensor batched matrix outputs; `arrow_svd_decompose` still returns NumPy because the admitted Rust Arrow SVD contract is ndarray-native | Partial | Keep Arrow-native egress wherever the Rust facade already defines an Arrow-native contract, and preserve typed ndarray-native egress where Rust does the same |
...
2. Missing Arrow/PyArrow breadth: large admitted portions of `nabled::arrow` remain unbound beyond the landed real dense/vector/matrix/stats slice.
