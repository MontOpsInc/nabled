...
1. `N-PY-003`: Continue the Python API/numerics parity pass under the locked `docs/PYNABLED_ARCHITECTURE.md` contract, focusing next on the remaining sparse format/product/iterative breadth beyond the landed CSR + reuse foundation, the remaining admitted dense/batched complex gaps, and the narrower remaining result-fidelity gaps around config/result-rich iterative workflows.
