...
1. `N-PY-003`: Continue the Python API/numerics parity pass under the locked `docs/PYNABLED_ARCHITECTURE.md` contract, focusing next on the remaining `f32` gaps (tensor breadth, callable `jacobian`/`optimization`, and Arrow-admitted rows), sparse breadth/reuse paths beyond the landed CSR carrier foundation, remaining dense complex parity, and the narrower remaining result-fidelity gaps around reusable sparse factorization/preconditioner objects plus config/result-rich iterative workflows.
