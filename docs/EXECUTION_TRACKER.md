...
1. `N-PY-003`: Continue the Python API/numerics parity pass under the locked `docs/PYNABLED_ARCHITECTURE.md` contract, focusing next on the remaining result-fidelity gaps around config/result-rich iterative workflows and any dense parity rows that are not blocked on later provider/backend feature exposure.
