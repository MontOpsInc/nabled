...
1. `N-PY-003`: Continue the Python API/numerics parity pass under the locked `docs/PYNABLED_ARCHITECTURE.md` contract, focusing next on the remaining admitted dense/batched complex gaps, the narrower remaining sparse solver/config breadth beyond the landed CSR/CSC/COO + reuse surface, and the remaining result-fidelity gaps around config/result-rich iterative workflows.
