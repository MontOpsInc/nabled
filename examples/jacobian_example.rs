...
fn main() -> Result<(), JacobianError> {
    ...
    let f_scalar = |x: &DVector<f64>| -> Result<f64, String> {
        Ok(x[0] * x[0] + x[1] * x[1])
    };
    ...
    let f_cubic = |x: &DVector<f64>| -> Result<f64, String> {
    ...
    let f_sphere = |x: &Array1<f64>| -> Result<f64, String> {
        Ok(x[0] * x[0] + x[1] * x[1] + x[2] * x[2])
    };
