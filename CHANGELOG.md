...
- `nabled-embeddings` crate and `embeddings` facade feature: normalize, score, rerank, kNN,
  PCA compress, `CorpusWorkspace`, eval metrics, MMR, int8 quantization, Arrow rerank wrappers
- `pynabled.embeddings` Python module (default wheels)
- Post-publish crates.io co-owner automation in `release.yml`

### Fixed

- `just prepare-release` now bumps `nabled-embeddings` workspace dependency pins
