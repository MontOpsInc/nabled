...
fn benchmark_nalgebra_svd(c: &mut Criterion) {
    ...
    for size in &[10, 50, 100, 200] {
        ...
        _ = group.bench_with_input(BenchmarkId::new("full_svd", size), size, |b, _| {
            b.iter(|| nalgebra_svd::compute_svd(black_box(&matrix)));
        });

        _ = group.bench_with_input(BenchmarkId::new("truncated_svd_k=5", size), size, |b, _| {
            b.iter(|| nalgebra_svd::compute_truncated_svd(black_box(&matrix), 5));
        });
    }
...
fn benchmark_ndarray_svd(c: &mut Criterion) {
    ...
    for size in &[10, 50, 100, 200] {
        ...
        _ = group.bench_with_input(BenchmarkId::new("truncated_svd_k=5", size), size, |b, _| {
            b.iter(|| ndarray_svd::compute_truncated_svd(black_box(&matrix), 5));
        });
    }
...
fn benchmark_comparison(c: &mut Criterion) {
    ...
    for size in &[10, 50, 100] {
        ...
        _ = group.bench_with_input(BenchmarkId::new("nalgebra", size), size, |b, _| {
            b.iter(|| nalgebra_svd::compute_svd(black_box(&nalgebra_matrix)));
        });

        _ = group.bench_with_input(BenchmarkId::new("ndarray", size), size, |b, _| {
            b.iter(|| ndarray_svd::compute_svd(black_box(&ndarray_matrix)));
        });
    }
