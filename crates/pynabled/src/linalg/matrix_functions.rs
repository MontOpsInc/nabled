...
pub fn matrix_exp<'py>(
    ...
    match utils::numeric_array2(matrix, "matrix")? {
        ...
        utils::NumericReadonlyArray2::C64(arr) => {
            let tol = tolerance.unwrap_or(DEFAULT_TOLERANCE);
            let result = nabled_linalg::matrix_functions::matrix_exp_complex_view(
                &arr.as_array(),
                terms,
                tol,
            )
...
pub fn matrix_log_eigen<'py>(py: Python<'py>, matrix: &Bound<'py, PyAny>) -> PyResult<Py<PyAny>> {
    match utils::numeric_array2(matrix, "matrix")? {
        ...
        utils::NumericReadonlyArray2::C64(arr) => {
            let result =
                nabled_linalg::matrix_functions::matrix_log_eigen_complex_view(&arr.as_array())
                    .map_err(to_py_err)?;
            Ok(utils::pyarray2_from_owned(py, result))
        }
    }
...
pub fn matrix_log_svd<'py>(py: Python<'py>, matrix: &Bound<'py, PyAny>) -> PyResult<Py<PyAny>> {
    match utils::numeric_array2(matrix, "matrix")? {
        ...
        utils::NumericReadonlyArray2::C64(arr) => {
            let result =
                nabled_linalg::matrix_functions::matrix_log_svd_complex_view(&arr.as_array())
                    .map_err(to_py_err)?;
            Ok(utils::pyarray2_from_owned(py, result))
        }
    }
...
pub fn matrix_power<'py>(
    ...
    match utils::numeric_array2(matrix, "matrix")? {
        ...
        utils::NumericReadonlyArray2::C64(arr) => {
            let result =
                nabled_linalg::matrix_functions::matrix_power_complex_view(&arr.as_array(), power)
                    .map_err(to_py_err)?;
            Ok(utils::pyarray2_from_owned(py, result))
        }
    }
...
pub fn matrix_sign<'py>(py: Python<'py>, matrix: &Bound<'py, PyAny>) -> PyResult<Py<PyAny>> {
    match utils::numeric_array2(matrix, "matrix")? {
        ...
        utils::NumericReadonlyArray2::C64(arr) => {
            let result = nabled_linalg::matrix_functions::matrix_sign_complex_view(&arr.as_array())
                .map_err(to_py_err)?;
            Ok(utils::pyarray2_from_owned(py, result))
        }
    }
