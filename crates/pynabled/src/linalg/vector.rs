...
pub fn dot(a: &Bound<'_, PyArray1<f64>>, b: &Bound<'_, PyArray1<f64>>) -> PyResult<f64> {
    ...
    nabled_linalg::vector::dot(&a_arr.as_array().to_owned(), &b_arr.as_array().to_owned())
        .map_err(to_py_err)
}
...
pub fn l2_norm(v: &Bound<'_, PyArray1<f64>>) -> PyResult<f64> {
    let arr = v.readonly();
    nabled_linalg::vector::l2_norm(&arr.as_array().to_owned()).map_err(to_py_err)
}
...
pub fn cosine_similarity(
    ...
    let a_arr = a.readonly();
    let b_arr = b.readonly();
    nabled_linalg::vector::cosine_similarity(
...
pub fn pairwise_l2_distance<'py>(
    ...
    let result = nabled_linalg::vector::pairwise_l2_distance(
        &l.as_array().to_owned(),
        &r.as_array().to_owned(),
    )
    .map_err(to_py_err)?;
...
pub fn pairwise_cosine_similarity<'py>(
    ...
    let l = left.readonly();
    let r = right.readonly();
    let result = nabled_linalg::vector::pairwise_cosine_similarity(
        &l.as_array().to_owned(),
        &r.as_array().to_owned(),
    )
    .map_err(to_py_err)?;
