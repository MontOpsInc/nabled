...
/// Solve Sylvester equation `A X + X B = C` using MAGMA mixed-precision iterative refinement.
#[pyfunction(name = "sylvester_solve_mixed")]
pub fn solve_sylvester_mixed<'py>(
    py: Python<'py>,
    matrix_a: &Bound<'py, PyAny>,
    matrix_b: &Bound<'py, PyAny>,
    matrix_c: &Bound<'py, PyAny>,
    ...
    match (
        ...
        (
            ...
            let result = nabled_linalg::sylvester::solve_sylvester_mixed_f64_view(
                &a.as_array(),
                &b.as_array(),
                &c.as_array(),
            )
            .map_err(to_py_err)?;
            Ok((utils::pyarray2_from_owned(py, result.solution), result.refinement_iterations))
        }
        (
            utils::NumericReadonlyArray2::C64(a),
            utils::NumericReadonlyArray2::C64(b),
            ...
            let result = nabled_linalg::sylvester::solve_sylvester_mixed_complex_view(
                &a.as_array(),
                &b.as_array(),
                &c.as_array(),
            )
        ...
        _ => Err(utils::matching_mixed_provider_dtype_error(&["matrix_a", "matrix_b", "matrix_c"])),
    }
}
...
/// Solve Lyapunov equation `A X + X A^T + Q = 0` using MAGMA mixed-precision iterative
/// refinement.
#[pyfunction(name = "lyapunov_solve_mixed")]
pub fn solve_lyapunov_mixed<'py>(
    py: Python<'py>,
    a: &Bound<'py, PyAny>,
    q: &Bound<'py, PyAny>,
) -> PyResult<(Py<PyAny>, usize)> {
    match (utils::numeric_array2(a, "a")?, utils::numeric_array2(q, "q")?) {
        (utils::NumericReadonlyArray2::F64(a_arr), utils::NumericReadonlyArray2::F64(q_arr)) => {
            let result = nabled_linalg::sylvester::solve_lyapunov_mixed_f64_view(
                &a_arr.as_array(),
                &q_arr.as_array(),
            )
            .map_err(to_py_err)?;
        ...
        (utils::NumericReadonlyArray2::C64(a_arr), utils::NumericReadonlyArray2::C64(q_arr)) => {
            let result = nabled_linalg::sylvester::solve_lyapunov_mixed_complex_view(
                &a_arr.as_array(),
                &q_arr.as_array(),
            )
            .map_err(to_py_err)?;
        ...
        _ => Err(utils::matching_mixed_provider_dtype_error(&["a", "q"])),
    }
}
