...
8. `accelerator-wgpu`: enables bounded WGPU-backed dense/vector/tensor kernel paths (`f32` native, `f64` native when `SHADER_F64` is available).
...
```toml
[dependencies]
nabled-linalg = { version = "0.0.10", features = ["openblas-system"] }
```
