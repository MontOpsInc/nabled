...
/// Ndarray vector primitives.
pub mod ndarray_vector {
    use super::*;
...
mod tests {
    ...
    fn dot_and_norm_are_correct() {
        ...
        let norm = ndarray_vector::l2_norm(&a).unwrap();
    ...
    fn cosine_similarity_works() {
        ...
        let similarity = ndarray_vector::cosine_similarity(&a, &b).unwrap();
        let distance = ndarray_vector::cosine_distance(&a, &b).unwrap();
    ...
    fn pairwise_l2_distance_matches_expected() {
        ...
        let distance = ndarray_vector::pairwise_l2_distance(&left, &right).unwrap();
    ...
    fn pairwise_cosine_workspace_reuse() {
        ...
        let mut output = ndarray::Array2::<f64>::zeros((left.nrows(), right.nrows()));
        let mut workspace = PairwiseCosineWorkspace::default();
    ...
    fn errors_on_bad_inputs() {
        ...
        let v = arr1(&[1.0_f64]);
        assert!(matches!(ndarray_vector::dot(&empty, &v), Err(VectorError::EmptyInput)));
