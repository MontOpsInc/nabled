...
use nabled_embeddings::{CorpusWorkspace, Metric, quantize_rows, query_corpus_scores, rerank};
...
fn bench_query_corpus_scores(c: &mut Criterion) {
    ...
    for n_corpus in [256_usize, 1024, 4096] {
        ...
        for metric in [Metric::Cosine, Metric::Dot, Metric::L2] {
            ...
            _ = group.bench_with_input(BenchmarkId::new("scores", &id), &n_corpus, |bench, _| {
                bench.iter(|| query_corpus_scores(black_box(&queries), black_box(&corpus), metric));
            });
        }
    }
    group.finish();
...
fn bench_rerank(c: &mut Criterion) {
    ...
    for n_candidates in [256_usize, 1024, 4096] {
        ...
        _ = group.bench_with_input(BenchmarkId::new("rerank", &id), &n_candidates, |bench, _| {
            bench.iter(|| {
                rerank(black_box(&query.view()), black_box(&candidates.view()), 10, Metric::Cosine)
            });
        });
    }
    group.finish();
