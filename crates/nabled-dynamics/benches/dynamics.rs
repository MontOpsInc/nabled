...
use nabled_dynamics::config::DynamicsConfig;
...
fn benchmark_dynamics(c: &mut Criterion) {
    ...
    let _ = group.bench_function("mass_matrix", |bench| {
        bench.iter(|| {
            mass_matrix(black_box(&model), black_box(&chain), black_box(&q), black_box(&config))
                .expect("mass matrix")
        });
    });
    let _ = group.bench_function("forward_dynamics", |bench| {
        bench.iter(|| {
